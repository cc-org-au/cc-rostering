    ...
    <Overlay onClose={onClose}>
      <ModalBox>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <h3 style={{margin:0,fontSize:18,fontWeight:700,color:"var(--text-primary)"}}>{isNew?"New project":"Edit project"}</h3>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            ...
            {totalInput&&parseFloat(totalInput)>0&&(
              <div style={{marginTop:5,fontSize:12,color:"var(--text-muted)"}}>{totalUnit==="days"?<>= <b style={{color:"var(--text-primary)"}}>{parseFloat(totalInput)*HPD}h</b> total</>:<>= <b style={{color:"var(--text-primary)"}}>{(parseFloat(totalInput)/HPD).toFixed(1)} days</b> equiv</>}</div>
            )}
            {budget&&cor&&parseFloat(budget)>0&&parseFloat(cor)>0&&(
              <div style={{marginTop:4,fontSize:12,color:"var(--text-muted)"}}>💡 Budget implies <b style={{color:"var(--text-primary)"}}>{Math.round(parseFloat(budget)/parseFloat(cor)/HPD)} days</b> ({Math.round(parseFloat(budget)/parseFloat(cor))}h) at ${cor}/hr</div>
            )}
          </div>
          <div>
            ...
            <div style={{marginTop:10}}>
              {staffMode==="flexible"
                ?<div style={{fontSize:13,color:"var(--text-muted)",padding:"10px 14px",background:"var(--bg-muted)",border:"1.5px solid #e5e7eb",borderRadius:8}}>Staff can vary day to day — assign whoever is needed when building the roster.</div>
          ...
          <div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,padding:10,border:"1.5px solid #e5e7eb",borderRadius:8,background:"var(--bg-surface)"}}>
          ...
          {localMonths.length===0
            ...
            :<div>
              ...
              <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"center",marginBottom:10,padding:"10px 14px",background:"var(--bg-muted)",border:"1.5px solid #e5e7eb",borderRadius:8}}>
                    ...
                    <div key={key} style={{background:hasOv?"#fffbeb":"var(--bg-muted)",border:`1.5px solid ${hasOv?"#fcd34d":"var(--border)"}`,borderRadius:8,padding:"10px 12px"}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                        <input type="number" min={0} value={displayVal} style={inpSt({width:64,padding:"6px 8px",textAlign:"center"})}
                          onChange={e=>setMonthVal(key,e.target.value)} onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border-input)"}/>
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <h3 style={{margin:0,fontSize:16,fontWeight:600,color:"var(--text-primary)"}}>Projects <span style={{fontWeight:400,color:"var(--text-muted)"}}>({projects.length})</span></h3>
