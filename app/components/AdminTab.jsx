...
import {
  listStubApiKeysForUi,
  createStubApiKey,
  revokeStubApiKey,
} from "../../lib/stubApiKeys";
import {
...
export default function AdminTab({ auth, showToast }) {
  ...
  const [hasSessionToken, setHasSessionToken] = useState(null);
  ...
  async function createApiKey(e) {
    ...
    if (!token) {
      ...
      setRevealedKey({
        ...
        name: row.name,
        warning:
          "Local stub key — saved only in this browser. Sign in as an admin to create server-side keys.",
      });
      setNewKeyName("");
      setKeys(listStubApiKeysForUi());
      showToast("Stub key created — copy it now (not synced to the server).");
      return;
    }
    try {
      const res = await fetch("/api/admin/api-keys", {
  ...
  async function revokeKey(id) {
    ...
    if (String(id).startsWith("stub-")) {
      revokeStubApiKey(id);
      showToast("Stub key removed from this browser.");
      setKeys(listStubApiKeysForUi());
      return;
    }
    const token = await getAccessToken(auth);
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16 }}>
        Manage users and programmatic API access. Server API keys are hashed; the secret is shown only once when created.
      </p>
      ...
      {sub === "apikeys" && (
        <div>
          {hasSessionToken === false && (
            ...
            >
              <strong>No admin session.</strong> You can still create <strong>local stub</strong> keys (saved in this browser only).
              Sign in with an admin account to create real keys on the server.
            </div>
          )}
          {revealedKey && (
            <div
              style={{
                ...cardSt({ marginBottom: 20, borderLeft: "4px solid var(--success-text)", background: "var(--info-bg)" }),
              }}
            >
