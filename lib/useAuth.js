...
export function useAuthProvider() {
  ...
  // Never stay on "Loading…" forever if Supabase hangs or init throws
  useEffect(() => {
    if (!loading) return;
    const id = setTimeout(() => {
      setLoading(false);
      console.warn("[auth] Initialization took too long — showing login screen.");
    }, 10000);
  ...
  useEffect(() => {
    ...
    const applyTestUser = (tu) => {
      if (!tu?.id) throw new Error("Invalid test user: missing id");
    ...
    const handleTestLogin = (e) => {
      try {
        ...
        console.error("testUserLogin error:", err);
      }
    };
    ...
    if (testUser) {
      try {
        ...
        console.error("Invalid stored test user — clearing", err);
        try {
      ...
      (async () => {
        try {
          ...
          if (session?.user) {
            ...
            // Do not await — a slow/hanging app_users query must not block leaving the loading screen.
      ...
      const { data } = supabase.auth.onAuthStateChange(async (event, session) => {
        ...
        if (tu) {
          try {
            ...
            console.error("onAuthStateChange test user:", err);
          }
          return;
        }
        setUser(null);
